...
class TestURL : XCTestCase {
    ...
    func test_URLByResolvingSymlinksInPath() {
        ...
        #elseif os(Android)
        do {
            // /etc is symlinked to /system/etc on Android
            let url = URL(fileURLWithPath: "/etc/..")
            let result = url.resolvingSymlinksInPath().absoluteString
            XCTAssertEqual(result, "file:///system/")
        }
        #else
        do {
...
#if os(Android)
        do {
            let url = URL(fileURLWithPath: "/system/ABC/..")
            let result = url.resolvingSymlinksInPath().absoluteString
            XCTAssertEqual(result, "file:///system/")
        }
#else
    ...
    func test_reachable() {
#if os(Android)
        var url = URL(fileURLWithPath: "/system")
#else
...
#if os(Android)
        var nsURL = NSURL(fileURLWithPath: "/system")
#else
