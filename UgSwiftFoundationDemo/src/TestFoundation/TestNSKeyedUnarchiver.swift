...
class TestNSKeyedUnarchiver : XCTestCase {
    ...
    private func test_unarchive_from_file(_ filename : String, _ expectedObject : NSObjectType) {
        guard let testFilePath = testBundle().path(forResource: filename, ofType: "plist") else {
            XCTFail("Could not find \(filename)")
            return
        }
        let object = NSKeyedUnarchiver.unarchiveObject(withFile: testFilePath) as? NSObjectType
        if let obj = object {
