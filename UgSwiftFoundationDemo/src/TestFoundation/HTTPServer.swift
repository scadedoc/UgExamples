...
import ScadeKit
    ...
    import CoreFoundation
 	//   import SwiftFoundation //- Alex, should i comment out this?
    import Darwin
    //import SwiftXCTest
#endif
...
class _TCPSocket {
...
#if os(Android)
            _ = try attempt("bind", valid: isZero, bind(listenSocket, addr, Int32(socklen_t(MemoryLayout<sockaddr>.size))))
#else
    ...
    private func createSockaddr(_ port: UInt16) -> sockaddr_in {
        ...
        #elseif os(Android)
            return sockaddr_in(sin_family: sa_family_t(AF_INET), sin_port: htons(port), sin_addr: in_addr(s_addr: addr), __pad: (0,0,0,0,0,0,0,0))
        #else
            return sockaddr_in(sin_len: 0, sin_family: sa_family_t(AF_INET), sin_port: CoreFoundation.CFSwapInt16HostToBig(port), sin_addr: in_addr(s_addr: addr), sin_zero: (0,0,0,0,0,0,0,0))
        #endif
    }
...
class _HTTPServer {
    ...
    public func respond(with response: _HTTPResponse, startDelay: TimeInterval? = nil, sendDelay: TimeInterval? = nil, bodyChunks: Int? = nil) throws {
        ...
        globalDispatchQueue.asyncAfter(deadline: deadlineTime) {
            do {
...
public class TestURLSessionServer {
	...
	if uri == "/UnitedStates" {
            ...
            let ip = hostComponents[0]

            var port = "1000"
            if (hostComponents.count > 1) {
                port = hostComponents[1]
            }
