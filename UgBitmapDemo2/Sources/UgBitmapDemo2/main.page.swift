...
class MainPageAdapter: SCDLatticePageAdapter {
	...
	override func load(_ path: String) {		
		...
		//main.onClick.append(SCDWidgetsEventHandler{_ in 
			//self.navigation!.go("BindingAndScalePage.page")})
		
		
		// get reference to second bitmap and set bitmap
		if let bc2 = self.page!.getWidgetByName("image2") as? SCDWidgetsImage {
			bc2.url = "Assets/dog.jpg"
			bc2.contentPriority = false
		}
		
//		if let bc3 = self.page!.getWidgetByName("image3") as? SCDWidgetsImage {
